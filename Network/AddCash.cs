...
namespace Banking.Network
{
    internal class AddCash : SimpleEvent<AddCash>
    {
        ...
        public override void OnReceived() // Only Host Should Be Able To Add Cash And Then Sync It
        {
            ...
            {
                Misc.Msg($"[AddCash] Adding Cash To Player: {SenderId} - {Currency}");
                LiveData.Players.AddCashToPlayer(LiveData.Players.GetCurrencyType.SteamID, SenderId, Currency);

                Misc.Msg($"[AddCash] Sending New Sync Event To All Players - FROM HOST");
