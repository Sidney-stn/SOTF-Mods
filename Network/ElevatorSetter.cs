...
namespace SimpleElevator.Network
{
    internal class ElevatorSetter : MonoBehaviour, Packets.IPacketReader
    {
        public void ReadPacket(UdpPacket packet, BoltConnection fromConnection)
        {
            ...
            {
                Misc.Msg("[ElevatorSetter] [ReadPacket] Recived packet on server", true);
            }
            else
            {
                Misc.Msg("[ElevatorSetter] [ReadPacket] Recived packet on client", true);
            }
            ...
            if (SonsSdk.Networking.NetUtils.IsDedicatedServer == false)
            {
                ...
                {
                    Misc.Msg("[ElevatorSetter] [ReadPacket] Recived packet not meant for this player", true);
                    Misc.Msg($"[ElevatorSetter] [ReadPacket] Recived packet meant for: {toPlayerSteamId}", true);
                    return;
                }
            }
