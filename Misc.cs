...
namespace Warps
{
    internal class Misc
    {
        ...
        {
            if (Config.DebugLoggingIngameWarps.Value)
            {
                RLog.Msg(msg);
            }
        ...
        {
            dialogManager.QuitGameConfirmDialog.AddOnOption1ClickedCallback((Il2CppSystem.Action)Warps.OnLeaveWorld);  // Quit World Confirm Button Press
        ...
        public static void OnHostModeGottenCorrectly(object sender, EventArgs e)
        {
            ...
            if (hostMode == SimpleSaveGameType.MultiplayerClient || hostMode == SimpleSaveGameType.Multiplayer) 
            {
                Network.Manager.RegisterEvents();  // Register Network Events
            }
