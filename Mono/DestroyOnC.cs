...
namespace Banking.Mono
{
    ...
    public class DestroyOnC : MonoBehaviour
    {
        ...
        private void OnShakeComplete()
        {
            if (shakeObject.name.Contains(gameObject.name))
            {
                string uniqueId = shakeObject.GetComponent<ATMController>().UniqueId;
                if (uniqueId != null) { Prefab.ActiveATM.spawnedAtms.Remove(uniqueId); }
                Saving.Load.ModdedAtms.Remove(shakeObject);
                Destroy(shakeObject);
